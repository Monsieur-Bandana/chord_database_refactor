...
public class SoundCreator {
    ...
    public AudioInputStream generateSound() throws IOException, UnsupportedAudioFileException {
        ...
        for (int el : keyList) {
            String res = "/server/sound/" + el + ".wav";

            InputStream raw = SoundCreator.class.getResourceAsStream(res);
            if (raw == null) {
            ...
            try (InputStream is = new BufferedInputStream(raw)) {
                ...
                if (format == null) {
                    ...
                    if (!format.matches(ais.getFormat())) {
                        ais.close();
                        throw new IllegalArgumentException("Unterschiedliches Audio-Format bei: " + is);
                    }
                }
